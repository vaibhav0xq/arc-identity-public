  ...
  return (
    <main className="relative min-h-screen w-full max-w-full">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-[1760px] flex-col px-3 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(0.65rem,env(safe-area-inset-top))] sm:px-6 sm:py-4 lg:px-8 2xl:px-12">
        <header className="arc-surface sticky top-2 z-40 grid gap-2 rounded-2xl px-3 py-2.5 shadow-[0_18px_52px_rgba(0,0,0,0.38),0_0_42px_rgba(212,175,55,0.07),inset_0_1px_0_rgba(255,255,255,0.06)] sm:top-4 sm:px-4 md:gap-2.5 lg:min-h-[72px] lg:px-5 lg:py-2 xl:grid-cols-[210px_minmax(0,1fr)_minmax(280px,auto)] xl:items-center xl:gap-3">
          <div className="grid gap-2 md:grid-cols-[minmax(190px,1fr)_auto] md:items-center xl:contents">
          ...
          <nav className="grid w-full min-w-0 grid-cols-2 gap-1.5 text-[0.76rem] font-medium text-slate-400 sm:grid-cols-4 md:grid-cols-4 lg:grid-cols-5 xl:col-start-2 xl:flex xl:items-center xl:justify-center xl:gap-1">
            {navItems.map(([label, href]) => (
              <Link
                key={href}
                href={href}
                className="min-w-0 whitespace-nowrap rounded-lg border border-white/[0.06] bg-white/[0.025] px-2.5 py-2 text-center transition-all duration-200 hover:bg-white/[0.07] hover:text-slate-100 hover:shadow-[0_0_20px_rgba(45,212,191,0.06)] xl:border-0 xl:bg-transparent xl:px-3 xl:py-2"
              >
                {label}
              </Link>
            ))}
            <ReportIssueLink className="min-w-0 whitespace-nowrap rounded-lg border border-amber-300/[0.10] bg-amber-300/[0.04] px-2.5 py-2 text-center text-amber-100/80 transition-all duration-200 hover:border-amber-300/20 hover:bg-amber-300/[0.08] hover:text-amber-50 xl:px-3" />
          </nav>
