...
export function ScoreRing({ score, size = "large", pulsing = false }: { score: number; size?: "large" | "small"; pulsing?: boolean }) {
  ...
  const glowSize = size === "large" ? "-inset-5 sm:-inset-10" : "-inset-5";
  ...
  return (
    <div className={`relative max-w-full shrink-0 ${pulsing ? "score-ring-refreshing" : ""}`}>
      <div className={`absolute ${glowSize} animate-score-pulse rounded-full bg-[radial-gradient(circle,rgba(212,175,55,0.22),rgba(161,105,197,0.12)_42%,transparent_68%)] blur-3xl`} />
      <div className={`absolute ${ringBorderSize} rounded-full border border-emerald-300/[0.12] bg-emerald-300/[0.04]`} />
      <div
        ...
        aria-label={`ARC Score ${clampedScore}`}
      >
        <div className="text-center">
          <div className={`${scoreText} font-black tabular-nums text-white drop-shadow-[0_2px_8px_rgba(255,255,255,0.12)]`}>{displayScore}</div>
          <div className="mt-0.5 text-[0.65rem] font-bold uppercase tracking-[0.28em] text-emerald-200/70">
            ARC Score
          </div>
        </div>
      </div>
