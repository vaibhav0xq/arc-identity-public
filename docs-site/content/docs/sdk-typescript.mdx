...
description: First-party TypeScript client for the Kyro API. Typed methods for all 9 operations, a shared error model and rate limit metadata.
---
...
The package is published to npm as [`@kyrodev/sdk`](https://www.npmjs.com/package/@kyrodev/sdk) (v0.1.0, MIT). Install:

```bash
npm install @kyrodev/sdk
```

Everything below works against the npm package or a source build.
...
`intake.start` is idempotent by state: `already_indexed` answers with no work done, `indexing` joins an in-flight scan for free and `started` begins a new scan for 8 rate units anonymously or 5 with an API key. See [intake](/docs/intake).
